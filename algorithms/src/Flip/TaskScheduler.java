...
//        System.out.println(leastInterval(new char[]{'A','A','A','B','B','B'}, 50)); //104
    ...
    public static int leastInterval(char[] tasks, int n) {
        ...
        int[] task = new int[26]; //alphabet consist of 26 characters
        for(char t : tasks) task[t-'A']++;

        PriorityQueue<Integer> pq = new PriorityQueue<>((p1, p2) -> p2-p1);

        for(int i=0 ; i<26; i++) if(task[i] > 0) pq.offer(task[i]);
        ...
        while(!pq.isEmpty()) {
            ...
            for(int i = 0; i <= n; i++) {
                if(!pq.isEmpty()) {
                    working[workingCount] = pq.poll();
                    workingCount++;
                }
                count++;
            }

            //reduce count and offer priorityqueue, if count==0, skip.
        ...
        //workingCount means the count of last turn
        return count - (n + 1) + workingCount;
    }
}
