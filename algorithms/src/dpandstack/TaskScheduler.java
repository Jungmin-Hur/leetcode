...
//        if(n==0) return tasks.length;
        ...
        while(!scheduler.isEmpty()) {
            int schedule = scheduler.poll();
            dic.put(schedule, dic.getOrDefault(schedule, 0) + 1);
            maxSchedule = schedule;
        }
//        int maxSchedule = scheduler.poll();
        int sameAsMaxScheduleCount = dic.get(maxSchedule);
//        while(!scheduler.isEmpty()) {
//            if(scheduler.poll() == maxSchedule) sameAsMaxScheduleCount++;
//            else break;
//        }
//        System.out.println(maxSchedule + ',' + (n+1) + ',' + sameAsMaxScheduleCount);
        return (maxSchedule -1) * (n+1) + sameAsMaxScheduleCount -1;
    }
}
